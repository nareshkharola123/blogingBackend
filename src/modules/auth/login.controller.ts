...
import {UserTenantRepository} from '../user-tenants/repositories/user-tenant.repository';
import {UserTenantPermissionRepository} from '../user-tenants/repositories/user-tenant-permission.repository';
...
export class LoginController {
  ...
  async login(
    ...
    try {
      ...
      return await this.createJWT(codePayload, this.client);
    } catch (error) {
  ...
  private async createJWT(
    ...
    try {
      ...
      } else if (userTenant.status !== StatusEnum.Active) {
      ...
      authUser.tenant = await this.tenantRepository.findById(
        userTenant.tenantId,
      );

      const role = await this.roleRepository.findById(userTenant.id);
